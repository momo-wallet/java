package com.mservice.processor.pay;

import com.mservice.pay.models.PayConfirmationResponse;
import com.mservice.pay.processor.notallinone.PayConfirmation;
import com.mservice.shared.sharedmodels.Environment;
import com.mservice.shared.sharedmodels.PartnerInfo;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.assertEquals;

class PayConfirmationTest {
    PartnerInfo devInfo = new PartnerInfo("MOMOLRJZ20181206", "mTCKt9W3eU1m39TW", "KqBEecvaJf1nULnhPF5htpG3AMtDIOlD");
    Environment env = new Environment("https://test-payment.momo.vn/pay/confirm", devInfo, "development");

    String publicKey = "";
    String privateKey = "";
    PayConfirmation payConfirmation = new PayConfirmation(env);
    long amount = 10000;
    String rollback = "revertAuthorize";
    String commit = "capture";
    String requestId = String.valueOf(System.currentTimeMillis());

    @Test
    void testRevertAuthorize() throws Exception {
        PayConfirmationResponse payConfirmationResponse = PayConfirmation.process(env, "1562299067267", rollback, requestId, "2305062978", "", "");

        assertEquals(103, payConfirmationResponse.getStatus(), "Wrong Response from MoMoServer");
    }

    @Test
    void tesCommitWrongTransId() throws Exception {
        PayConfirmationResponse payConfirmationResponse = PayConfirmation.process(env, "1562299067267", commit, requestId, "555", "", "");

        assertEquals(2131, payConfirmationResponse.getStatus(), "Wrong Response from MoMoServer");
    }

    @Test
    void testCommitFail() throws Exception {
        PayConfirmationResponse payConfirmationResponse = PayConfirmation.process(env, "1562309558787", commit, requestId, "2305066529", "", "");

        assertEquals(2132, payConfirmationResponse.getStatus(), "Wrong Response from MoMoServer");
    }

}